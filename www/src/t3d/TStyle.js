...
export class TFBlok  {
	constructor(par, key) {
		...
		this.setObj=function(_lebel){			
			...
			for (i = 0; i < _lebel._text.length; i++) {

				if(this.arSimvol[_lebel._text[i]]==undefined){
					this.arSimvol[_lebel._text[i]]=new TFSimvol(this, _lebel._text[i])
				}
				_lebel.cont3d.add(this.arSimvol[_lebel._text[i]].getMesh(_lebel.material))
				if(par.ng!=undefined)_lebel.cont3d.children[i].position.y=-par._whText
				if(i!=0){
					_lebel.cont3d.children[i].position.x=_lebel.cont3d.children[i-1].position.x+_lebel.cont3d.children[i-1].w+this.par._whText/200;
					
					_lebel._widthGeom=_lebel.cont3d.children[i].position.x+_lebel.cont3d.children[i].w;
				}else{
					_lebel.cont3d.children[i].position.x=0;
					_lebel._widthGeom=0;
				}				
			}							
		}
